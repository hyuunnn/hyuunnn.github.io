...
~~`Settings -> Editor -> Code Style -> Java -> Wrapping and Braces -> Chained method calls: Wrap if long -> Wrap always`~~ (2023-02-12 기준 google style 기본 설정 사용 - Wrap if long)
...
윈도우는 CRLF(\r\n), Unix/Linux는 LF(\n)으로 되어있다. 보통 LF로 통일하여 사용하는 것 같다.

## 필요없는 객체 관련 팝업 삭제하기

클래스를 만들 때마다 `no usages, 3 usages, 1 inheritor` 등등 뜨면서 라인을 하나씩 더 잡아먹는다.. 제한된 화면에서 라인을 차지하는게 불필요하다고 생각이 들었다.

굳이 알려주지 않아도 사용하지 않는 메서드, 클래스는 진한 회색으로 바뀌고 usages 정보는 `Ctrl + 왼쪽 클릭` 기능을 사용하면 된다.
...
### assertDoesNotThrow snippet

*Template text*

```java
org.junit.jupiter.api.Assertions.assertDoesNotThrow($EXCEPTION$.class, () -> {
  $MOVE_CODE$$END$
});
```

`Abbreviation: doesnot`, `Description: Assertions.assertDoesNotThrow`

나머지 설정은 assertThrows와 동일하게 설정
...
`Abbreviation: try`, `Description: try catch`
