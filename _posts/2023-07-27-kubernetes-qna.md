...
* 왜 파드가 여러개 존재하는가?
    ...
    * 대규모 서비스의 경우 서비스가 죽어서 잠깐이라도 서비스가 안된다면 이는 엄청난 손실이기 때문에 자동으로 재배포하는 과정이 중요하다. 또한 롤링 업데이트를 통해 다운타임이 없는 배포가 가능하며, 배포된 어플리케이션에 문제가 발생했을 때도 빠른 시간 안에 롤백이 가능하다. 롤백 시에도 롤링 업데이트와 동일한 과정으로 무중단 배포가 유지된다. (잠깐 멈추는 것 자체가 문제 - <a href="https://tv.kakao.com/channel/3693125/cliplink/423597081">카카오페이 사례)
        * ```
