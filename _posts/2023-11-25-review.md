...
* <a href="https://jojoldu.tistory.com/609">자바 공화국</a>
  * 공감이 많이 되는 글이었다. 국내에는 자바 스프링을 쓰는 기업이 많고, 관련 책들도 자바 스프링을 기반으로 설명하는 책들이 많이 있다. 그러나 책에서 설명하는 기법이나 핵심 내용, 컨셉들은 바뀌지 않는다. (Java라서 다르고, Python이라서 다른게 아니라는 의미) 특히 이번에 장고 프레임워크를 공부하면서 MTV(Model-Template-View) - MVC와 유사한 패턴, 지연 로딩, N+1 문제와 같은 키워드들이 이전에 자바 스프링을 공부하면서 배웠던 내용이라서 수월하게 공부할 수 있었다.
